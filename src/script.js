document.addEventListener('DOMContentLoaded', () => {
    ...
    function setupThemeManager() {
        const themeButtons = {
            ...
            // dark theme handled by React app; legacy button no-op
    ...
    function renderFavoriteTabsAndContent(allFavoriteApps) {
        ...
        tabsList.className = 'flex border-b border-muted flex-wrap';
    ...
    function getResourceBorder(tipo){ const t=(tipo||'').toLowerCase(); if(t.includes('imagen')||t.includes('image')) return 'border-fuchsia-500'; if(t.includes('video')) return 'border-rose-500'; if(t.includes('audio')) return 'border-emerald-500'; return 'border-slate-400'; }
function createAppCard(app) { const card = document.createElement('div'); card.className = `card bg-surface rounded-lg shadow-sm overflow-hidden flex flex-col ${getResourceBorder(app.tipo_recurso)}`; const isFav = isFavorite(app.key); const favClass = isFav ? 'text-red-500 fill-red-500' : 'text-gray-400 hover:text-red-500'; const fullDescription = app.descripcion_app || 'No hay descripción.'; let descriptionHtml; if (fullDescription.length > 350) { const shortDescription = fullDescription.substring(0, 350).trim(); descriptionHtml = `<div class="description-container text-muted text-sm mb-4 flex-grow"><p class="short-desc">${shortDescription}... <a href="#" class="read-more font-semibold text-[var(--accent)] hover:underline ">más</a></p><p class="long-desc hidden">${fullDescription} <a href="#" class="read-less font-semibold text-[var(--accent)] hover:underline ">menos</a></p></div>`; } else { descriptionHtml = `<p class="text-muted text-sm mb-4 flex-grow">${fullDescription}</p>`; } card.innerHTML = `<div class="p-5 flex-grow flex flex-col relative"><button class="favorite-btn absolute top-3 right-3 p-1 rounded-full bg-surface/50 hover:bg-surface/90 focus:outline-none focus:ring-2 focus:ring-[var(--accent)]" title="Gestionar favorito" data-key="${app.key}"><svg class="w-6 h-6 transition-colors duration-200 ${favClass}" fill="${isFav ? 'currentColor' : 'none'}" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4.318 6.318a4.5 4.5 0 016.364 0L12 7.636l1.318-1.318a4.5 4.5 0 016.364 6.364L12 20.364l-7.682-7.682a4.5 4.5 0 010-6.364z"></path></svg></button><p class="text-sm font-semibold text-muted mb-3 pr-8">${app.plataforma || 'Sin plataforma'}</p><h3 class="text-xl font-bold text-primary mb-2">${app.titulo_app}</h3><p class="text-sm text-muted mb-3">por <span class="font-medium">${createFilterLink(app.nombre_autor, 'nombre_autor')}</span></p>${descriptionHtml}<div class="text-xs text-muted space-y-1 mt-auto"><p><strong>Nivel:</strong> ${app.nivel_educativo ? createFilterLink(app.nivel_educativo, 'nivel_educativo') : ''}</p><p><strong>Área:</strong> ${app.area_conocimiento ? createFilterLink(app.area_conocimiento, 'area_conocimiento') : ''}</p><p><strong>Licencia:</strong> ${app.licencia || ''}</p></div></div><div class="p-4 border-t border-muted bg-surface/50"><div class="flex flex-wrap mb-3 min-h-[2rem]">${app.palabras_clave?.split(',').filter(k => k.trim()).map(k => `<span class="keyword-tag ${activeFilters.palabras_clave.has(k.trim()) ? 'active' : ''} text-xs font-medium mr-2 mb-2 px-2.5 py-0.5 rounded-full" data-keyword="${k.trim()}">${k.trim()}</span>`).join('') || ''}</div><a href="${app.url_app}" target="_blank" class="block w-full text-center bg-primary-dark text-white font-semibold py-2.5 px-4 rounded-lg hover:bg-primary transition-colors">Visitar Aplicación</a></div>`; return card; }
    ...
    function getPlatformStyle(platform) {
        const STYLES = {
            ...
            'GitHub Pages': 'border-black dark:border-muted',
