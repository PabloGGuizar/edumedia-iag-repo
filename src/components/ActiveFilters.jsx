        ...
        <div className="flex flex-wrap gap-2 mb-4">
            {Object.entries(activeFilters).map(([category, filterSet]) => (
                Array.from(filterSet).map(filterValue => (
                    <div key={`${category}-${filterValue}`} className="flex items-center bg-surface text-primary text-sm font-medium pl-3 pr-2 py-1 rounded-full  ">
                        <span>{getCategoryName(category)}: {filterValue}</span>
                        <button onClick={() => onRemoveFilter(category, filterValue)} className="ml-2 text-slate-500 hover:text-slate-800  dark:hover:text-white" title="Eliminar filtro">&times;</button>
                    </div>
                ))
            ))}
        </div>
