...
import { getResourceGradient } from '../utils';
...
export default function AppCard({ app }) {
  ...
  // Solo usar imagen_url si viene del formulario; si no, degradado
  useEffect(() => {
    const direct = (app?.imagen_url || '').trim();
    setImageSrc(direct || null);
  }, [app?.imagen_url]);

  return (
    <div className={`card rounded-lg shadow-sm overflow-hidden flex flex-col`}>
      <div className="w-full h-40 bg-surface/50">
        {imageSrc ? (
          <img
            src={imageSrc}
            alt={app.titulo_app || 'Recurso'}
            className="w-full h-full object-cover"
            loading="lazy"
            onError={() => setImageSrc(null)}
          />
        ) : (
          <div className={`w-full h-full ${getResourceGradient(app.tipo_recurso)}`}></div>
        )}
      </div>
      <div className="p-5 flex flex-col relative">
        ...
        <p className="text-sm font-semibold text-muted mb-2 pr-8">{app.plataforma || 'Sin plataforma'}</p>
        <div className="text-xs text-muted mb-2">{app.tipo_recurso || 'No especificado'}</div>
        ...
        <div className="description-container text-muted  text-sm mb-3">
      ...
      <div className="p-3 bg-surface/50 mt-1">
